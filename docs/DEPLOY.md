# Raffle Deployment Guide

This guide walks through deploying Raffle from a fresh clone to a working live demo. Two pieces deploy independently:
...
1. Picks a sample 32-byte secret and computes its sha256.
2. Builds the raffle contract.
3. Deploys with the hash as the commitment.
4. Logs the secret to the terminal so you can paste it into the reveal panel during the demo.
5. Writes `NEXT_PUBLIC_MAIN_CONTRACT_ID` into `.env.local`.
